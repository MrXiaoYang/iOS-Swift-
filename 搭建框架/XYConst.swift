//
//  XYConst.swift
//  XYDanTang
//
//  Created by apple on 16/11/14.
//  Copyright © 2016年 apple. All rights reserved.
//

import UIKit


enum XYOtherLoginButtonType: Int {
    case weiboLogin = 100
    case weChatLogin = 101
    case QQLogin = 102
}



/// 是否登录
let isLogin = "isLogin"

func XYColor(r:CGFloat, g:CGFloat, b:CGFloat, a:CGFloat) -> UIColor {
    return UIColor(red: r / 255.0, green: g / 255.0, blue: b / 255.0, alpha: a)
}

func XYGlobalRedColor() -> UIColor {
    return XYColor( r: 240, g: 80, b: 83, a: 1)
}


func XYGlobalColor() -> UIColor {
    return XYColor(r: 240, g: 240, b: 240, a: 1)
}

/// 新特性界面图片数量
let kNewFeatureCount = 4
/// 间距
let kMargin: CGFloat = 10.0/// 屏幕的宽
let SCREENW = UIScreen.main.bounds.size.width
/// 屏幕的高
let SCREENH = UIScreen.main.bounds.size.height
/// 我的界面头部图像的高度
let kXYMMineHeaderImageHeight: CGFloat = 200

/********* Me *************/

/// 首页顶部标签指示条的高度
let kIndicatorViewH: CGFloat = 2.0
/// 线宽
let klineWidth: CGFloat = 1.0
/// 动画时长
let kAnimationDuration = 0.25
/// 我的界面头部图像的高度
let kYMMineHeaderImageHeight: CGFloat = 200

/// 分类界面 顶部 item 的高
let kitemH: CGFloat = 75
/// 分类界面 顶部 item 的宽
let kitemW: CGFloat = 150
/// 圆角
let kCornerRadius: CGFloat = 5.0

// 分享按钮背景高度
let kTopViewH: CGFloat = 230

enum XYShareButtonType: Int {
    /// 微信朋友圈
    case WeChatTimeLine = 0
    /// 微信好友
    case WeChatSession = 1
    /// 微博
    case Weibo = 2
    /// QQ 空间
    case QZone = 3
    /// QQ 好友
    case QQFriends = 4
    /// 复制链接
    case CopyLinkp = 5
}


/// 我的界面头部图像的高度

/************ 网络相关 ************/
/// 服务器地址
let BASE_URL = "http://api.dantangapp.com/"
